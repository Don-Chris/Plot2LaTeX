...
| `'FontName'`|  'auto' (default), 'Helvetica', '' (do nothing), (fontName as a char)  |Option that changes the fontname of all text elements in the figure. This is often needed since matlab's SVG export doesn't save the text as text elements, instead it saves them as line segments which would result in the "No text elements found" warning message of Plot2LaTeX and an unsuccessful export. 'auto' changes the fontname to the default font name of matlab. '' (empty string) will result in no changes.|
